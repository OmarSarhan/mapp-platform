...
**The third vendored copy, deliberately duplicated.** The three components ship
as separate images with no shared package, and the scope document is explicit
that each trust boundary evaluates the scheme independently "so sharing a
defective helper cannot make a bad digest look correct everywhere". So this is a
copy, and tests/test_canonical.py checks all three against each other -- because
a copy nobody compares is a copy that has already drifted. Change one, change
all three, in the same commit.

This is the *asserting* boundary, and the asymmetry matters. This component
computes the digest and claims it; the configuration API recomputes it from the
request actually arriving and compares. The broker in between records whatever
it is handed and never recomputes anything. So a disagreement between this file
and the configuration API's copy does not fail here -- it fails there, as a
refusal of every token B with nothing to say why.

P10 makes this a versioned cross-component security contract rather than a
serialization convenience: this runtime, the broker and the configuration API each
