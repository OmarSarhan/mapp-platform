...
**A vendored copy of mcp-auth/canonical.py, deliberately duplicated.** The two
components ship as separate images with no shared package, and the scope
document is explicit that each trust boundary evaluates the scheme
independently "so sharing a defective helper cannot make a bad digest look
correct everywhere". So this is a copy, and
tests/test_canonicalization_agreement.py checks both copies against the RFC's
own vectors *and* against each other -- because a copy nobody compares is a
copy that has already drifted. Change one, change both, in the same commit.

This is the verifying boundary: the broker records whatever digest it is given
and never recomputes it, so this file is what decides whether a token B is
actually bound to the request being executed.

P10 makes this a versioned cross-component security contract rather than a
serialization convenience: mapp-mcp, the broker and this configuration API each
