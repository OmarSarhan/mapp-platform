...
function layerCatalogTable(layer,catalog,workspaceDb){
 const relations=[...new Set([layer?.table,...Object.values(layer?.tables||{})].filter(value=>typeof value==='string'&&value))];
 if(relations.length!==1)return undefined;
 return catalog.find(table=>table.dbs===(layer?.dbs||workspaceDb)&&`${table.schema}.${table.table}`===relations[0]);
}
...
function BasicLegendControls({layer,setLayer}) {
 ...
 if(!managed)return <><div className="subpanel"><h3>Layer symbol legend</h3><p className="muted style-note">The active theme controls this legend. Use the category controls above to edit each label and symbol; the default symbol is the fallback for unmatched features.</p></div><GeometrySwatchControls layer={layer} setLayer={setLayer}/></>;
...
export function Dashboard({openSecurity,openDerivedLayers,openSemantic,openFederation,onLogout=()=>{},derivedChange=null,activeArea="workspace",openWorkspace=()=>{},children}){
 ...
 const setLayer=layer=>mutateLayers((x,l)=>l.layers[selected]=layer), table=layerCatalogTable(layers[selected],catalog,ws?.dbs);
 ...
 return <div className="app-shell">
  ...
  <header className="app-header">
   <button className="brand-lockup" disabled={busy} onClick={()=>{openWorkspace();setSelected();setSidebarMode('layers')}} aria-label="Open workspace settings">
    <span className="brand-mark" aria-hidden="true">M</span>
    <span><strong>MAPP</strong><small>Platform console</small></span>
   </button>
   <PrimaryNavigation>
    <button className={activeArea==='workspace'&&sidebarMode==='layers'?'active':''} aria-current={activeArea==='workspace'&&sidebarMode==='layers'?'page':undefined} aria-label="Workspace" title="Workspace" disabled={busy} onClick={()=>{openWorkspace();setSelected();setSidebarMode('layers')}}><Layers3 size={17} aria-hidden="true"/><span>Workspace</span></button>
    <button className={activeArea==='workspace'&&sidebarMode==='catalog'?'active':''} aria-current={activeArea==='workspace'&&sidebarMode==='catalog'?'page':undefined} aria-label="Source catalog" title="Source catalog" disabled={busy} onClick={()=>{openWorkspace();setSidebarMode('catalog')}}><Database size={17} aria-hidden="true"/><span>Sources</span></button>
    <button className={activeArea==='derived'?'active':''} aria-current={activeArea==='derived'?'page':undefined} aria-label="Derived layers" title="Derived layers" disabled={busy} onClick={()=>openDerivedLayers?.({})}><Waypoints size={17} aria-hidden="true"/><span>Derived layers</span></button>
    <button className={activeArea==='semantic'?'active':''} aria-current={activeArea==='semantic'?'page':undefined} aria-label="Semantic catalog" title="Semantic catalog" disabled={busy} onClick={openSemantic}><Database size={17} aria-hidden="true"/><span>Semantic catalog</span></button>
    <button className={activeArea==='federation'?'active':''} aria-current={activeArea==='federation'?'page':undefined} aria-label="Federated sources" title="Federated sources" disabled={busy} onClick={openFederation}><Database size={17} aria-hidden="true"/><span>Federated sources</span></button>
    <button className={activeArea==='security'?'active':''} aria-current={activeArea==='security'?'page':undefined} aria-label="Access & audit" title="Access & audit" disabled={busy} onClick={openSecurity}><ShieldCheck size={17} aria-hidden="true"/><span>Access &amp; audit</span></button>
   </PrimaryNavigation>
   <div className="account-actions">
     <div className="reload-actions" role="group" aria-label="Reload controls">
  ...
  <div className="status-region dashboard-status" aria-live="polite" aria-atomic="true">
   {status&&<div className={`status-bar ${status.kind}`}><div><strong>{status.message}</strong>{status.errors?.length>0&&<ul>{status.errors.map((error,index)=><li key={index}>{error.path}: {error.message}</li>)}</ul>}</div><button aria-label="Dismiss status" onClick={()=>setStatus(null)}><X size={17} aria-hidden="true"/></button></div>}
  </div>
  <main className={`dashboard-main ${sidebarMode==='catalog'?'source-catalog-main':''}`} hidden={activeArea!=='workspace'} aria-busy={busy} inert={busy||activeArea!=='workspace'}>
   {sidebarMode==='catalog'?<SourceCatalog catalog={catalog} selected={selectedCatalog} onSelect={setSelectedCatalog} search={catSearch} setSearch={setCatSearch} addTable={addTable} namedReadOnly={namedReadOnly} openDerivedLayers={openDerivedLayers}/>:<>
   <button className={`sidebar-scrim ${sidebarOpen?'visible':''}`} aria-label="Close workspace navigator" onClick={()=>setSidebarOpen(false)}></button>
   <aside className={`workspace-sidebar ${sidebarOpen?'open':''}`}>
    <div className="sidebar-context"><span className="eyebrow">Workspace</span><h2>{loc?.name||ws.key}</h2><p>{Object.keys(layers).length} configured layers · {localeKey==='locale'?'Default locale':localeKey}</p></div>
    <div className="sidebar-panel" role="tabpanel">
     <button className={`workspace-nav-item ${!selected?'active':''}`} aria-pressed={!selected} onClick={()=>{setSelected();setSidebarOpen(false)}}><Settings2 size={17} aria-hidden="true"/><span><strong>Workspace settings</strong><small>Extent, view and locale defaults</small></span></button>
     <SearchField label="Filter configured layers" placeholder="Find a configured layer" value={search} onChange={event=>setSearch(event.target.value)}/>
     <LayerNavigation layers={layers} search={search} selected={selected} setSelected={key=>{setSelected(key);setSidebarOpen(false)}}/>
     {Object.keys(layers).length===0&&<p className="sidebar-empty">No layers yet. Add a spatial table from the source catalog.</p>}
    </div>
   </aside>
   <section id="editor" className="editor-shell">
    <div className="workspace-commandbar">
     <button className="icon-button rail-trigger" aria-label="Open workspace navigator" title="Open workspace navigator" onClick={()=>setSidebarOpen(true)}><PanelLeft size={19} aria-hidden="true"/></button>
     <div className="editor-heading"><span className="eyebrow">{layer?'Layer editor':'Configuration'}</span><h1>{layer?.name||'Workspace settings'}</h1><p>{layer?`${layer.dbs||ws.dbs} · ${layer.table||'non-database layer'}`:'Map extent, locale and workspace defaults'}</p></div>
     <div className="workspace-actions">
      {localeOptions.length>1&&<label className="locale-control"><span>Locale</span><select disabled={busy} value={localeKey} onChange={event=>{setSelectedLocale(event.target.value);setSelected()}}>{localeOptions.map(([key,value])=><option value={key} key={key}>{value?.name||key}</option>)}</select></label>}
...
function SourceCatalog({catalog,selected,onSelect,search,setSearch,addTable,namedReadOnly,openDerivedLayers}){
 const [database,setDatabase]=useState(''),[sourceKind,setSourceKind]=useState('all');
 const visible=catalog.filter(table=>(!database||table.dbs===database)&&(sourceKind==='all'||(table.schema==='derived_layers')===(sourceKind==='derived'))&&`${table.dbs}.${table.schema}.${table.table}`.toLowerCase().includes(search.toLowerCase()));
 const table=catalog.find(item=>`${item.dbs}.${item.schema}.${item.table}`===selected);
 const spatial=table?.columns.some(column=>column.geometryType);
 return <section className="source-catalog-page" aria-label="Source catalog">
  <div className="form-head"><div><span className="eyebrow">Data sources</span><h1>Source catalog</h1><p>Find a relation, inspect its columns, then add it to your workspace.</p></div><span>{visible.length} of {catalog.length} tables</span></div>
  <div className="catalog-toolbar">
   <SearchField label="Filter database tables" placeholder="Search database, schema or table" value={search} onChange={event=>setSearch(event.target.value)}/>
   <label><span>Database</span><select value={database} onChange={event=>setDatabase(event.target.value)}><option value="">All databases</option>{[...new Set(catalog.map(table=>table.dbs))].sort().map(name=><option key={name}>{name}</option>)}</select></label>
   <label><span>Relations</span><select value={sourceKind} onChange={event=>setSourceKind(event.target.value)}><option value="all">All relations</option><option value="source">Source relations</option><option value="derived">Derived relations</option></select></label>
  </div>
  <div className="catalog-browser">
   <div className="catalog-list" aria-label="Database tables">{visible.map(item=>{const key=`${item.dbs}.${item.schema}.${item.table}`;return <button aria-pressed={selected===key} className={`catalog-item ${selected===key?'active':''}`} key={key} onClick={()=>onSelect(key)}><strong>{item.schema}.{item.table}</strong><small>{item.dbs} · {item.columns.length} columns · {item.columns.filter(column=>column.geometryType).map(column=>column.geometryType).join(', ')||'No geometry'}</small></button>})}{!visible.length&&<p>No relations match these filters.</p>}</div>
   <section className="catalog-detail" aria-label="Selected source">{table?<>
    <div className="form-head"><div><span className="eyebrow">{table.dbs}</span><h2>{table.schema}.{table.table}</h2></div><button disabled={namedReadOnly||!spatial} title={namedReadOnly?'Switch to the default locale to add a layer.':!spatial?'A geometry column is required.':undefined} onClick={()=>addTable(table)}>Add selected table as layer</button></div>
    {table.schema==='derived_layers'&&<button onClick={()=>openDerivedLayers?.({name:table.table,action:'edit'})}>Edit derived definition</button>}
    <p className="muted">Column metadata only. Adding a layer creates an unsaved workspace edit.</p>
    <table className="source-columns"><thead><tr><th>Column</th><th>Type</th><th>Geometry / SRID</th></tr></thead><tbody>{table.columns.map(column=><tr key={column.name}><th scope="row">{column.name}</th><td>{column.type}</td><td>{column.geometryType?`${column.geometryType} · EPSG:${column.srid}`:'—'}</td></tr>)}</tbody></table>
...
function LayerSection({title:heading,summary,children,open=false,className='',sectionRef}){return <details ref={sectionRef} className={`layer-section ${className}`} open={open}><summary><span>{heading}</span><small>{summary}</small></summary><div className="layer-section-body">{children}</div></details>}
function PluginControls({scope,target,catalogue,onChange}) {
 ...
 const toggle=(plugin,enabled)=>{const next=clone(target),urls=new Set([plugin.entryUrl,...(plugin.aliases||[])]),sources=(next.plugins||[]).filter(url=>!urls.has(url));if(enabled){sources.push(plugin.entryUrl);next[plugin.configurationKey]??={}}else delete next[plugin.configurationKey];if(sources.length)next.plugins=sources;else delete next.plugins;if(scope==='locale'&&plugin.dispatch?.includes('sync')){let sync=[...(next.syncPlugins||[])];if(enabled&&!sync.includes(plugin.registrationKey))sync.push(plugin.registrationKey);if(!enabled)sync=sync.filter(key=>key!==plugin.registrationKey);if(sync.length)next.syncPlugins=sync;else delete next.syncPlugins}onChange(next)};
...
function Layer({workspace,localeKey,layerKey,layer,table,catalog,databases,icons,plugins,workspaceDb,setLayer,renameLayer,remove}){
 const appearanceRef=useRef(null),styledLayer=['cluster','mvt','geojson','vector','wkt'].includes(layer.format)&&!layer.template;
 ...
 return <div className="panel layer-editor"><div className="form-head"><div><h2>{layer.name||layerKey}</h2><p>{databaseLayer?`${activeDb} · ${layer.table}`:tileLayer?'Tile layer':'Advanced/external XYZ layer'}</p></div><div className="layer-heading-actions">{styledLayer&&<button type="button" onClick={()=>{appearanceRef.current.open=true;appearanceRef.current.scrollIntoView?.({block:'start',behavior:'smooth'})}}>Edit symbols and icons</button>}<button className="danger" onClick={remove}>Remove layer</button></div></div><div className="layer-section-list">
...
function Login({onLogin}){const [password,setPassword]=useState(''),[error,setError]=useState('');const submit=async e=>{e.preventDefault();setError('');try{const result=await api('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password})});csrfToken=result.csrfToken;sessionStorage.setItem('mapp-csrf',csrfToken);onLogin()}catch(err){setError(err.message)}};return <main className="login-shell"><form className="panel login-panel" onSubmit={submit}><span className="brand-mark" aria-hidden="true">M</span><span className="eyebrow">MAPP Platform</span><h1>Administration console</h1><p>Sign in to manage the workspace, data lifecycles, and agent access.</p><label><span>Administrator password</span><input autoFocus autoComplete="current-password" type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label>{error&&<div className="expression-result error">{error}</div>}<button>Sign in</button></form></main>}
...
function DerivedDraftStatus({draft}){
 if(!draft)return <small className="draft-state">Permanent relation</small>;
 if(draft.state==='adopted')return <small className="draft-state">Adopted · kept permanently</small>;
 if(draft.state!=='active')return null;
 return <small className="draft-state">Disposable draft · expires {draft.expiresAt ? new Date(draft.expiresAt).toLocaleString() : 'unknown'}{draft.lastCleanupReason&&<span> · Last check: {draft.lastCleanupReason.replaceAll('-',' ')}</span>}</small>;
}
function DraftCleanupStatus({status,error}){
 return <details className="draft-cleanup-status"><summary>Automatic draft cleanup{status?.cleanup?.workerRunning===true?' · running':status?.cleanup?.workerRunning===false?' · not running':''}</summary>
  <p>Unused disposable drafts are checked every {status?.cleanupIntervalSeconds||60} seconds and removed after expiry or rejection of their owning proposal. Published and permanent relations are retained.</p>
  <p className="muted">Live references, other pending proposals, active previews and database dependencies block deletion. Unverifiable state is retained for review.</p>
  {status?.cleanup?.lastCompletedAt&&<p>Last completed check: {new Date(status.cleanup.lastCompletedAt).toLocaleString()}</p>}
  {(error||status?.cleanup?.lastError)&&<p role="status">Cleanup status needs attention: {error||status.cleanup.lastError}. Relations are retained when checks cannot be completed.</p>}
  {status?.possiblyTruncated&&<p className="muted">Showing up to {status.limit} lifecycle records; the watcher continues in bounded batches.</p>}
 </details>;
}
...
export function DerivedLayers({close,active=true,initialName=null,initialAction='create',initialRequestKey=0,onChanged=result=>window.dispatchEvent(new CustomEvent('mapp-derived-layer-changed',{detail:result}))}){
 ...
 const [draftStatus,setDraftStatus]=useState(null),[draftError,setDraftError]=useState(''),[derivedSearch,setDerivedSearch]=useState('');
 useEffect(()=>{if(!active)return;let mounted=true;const refresh=async()=>{try{const data=await api('/api/derived-layers/drafts');if(mounted){setDraftStatus(data);setDraftError('')}}catch(error){if(mounted)setDraftError(error.message)}};refresh();const timer=setInterval(refresh,60000);return()=>{mounted=false;clearInterval(timer)}},[active]);
 ...
 useEffect(()=>{if(!initialName)return;if(initialAction==='delete')action({name:initialName},'drop');else edit({name:initialName})},[initialName,initialAction,initialRequestKey]);
...
export const TOKEN_SCOPE_OPTIONS=[
 ...
 {id:'derive',group:'Workspace',label:'Read derived layers',help:'Read managed derived relations and aggregate over them. Does not create or change one.'},
 {id:'derive:manage',group:'Workspace',label:'Manage derived layers',help:'Create, replace, refresh, and drop managed derived relations. Acts on the database directly, with no proposal to review first.'},
...
export const MCP_SCOPE_OPTIONS=[
 ...
 {id:'derive:manage',label:'Agent may build and remove derived layers',help:'Needed by derived_layers_create, _replace, _refresh and _drop. These act on the database directly -- there is no proposal to review first, which is why each one asks you before it happens. Dropping is refused outright while any layer still reads the relation, and the approval names what would break. The widest thing on this list: grant it to an agent you would let run DDL.'},
 {id:'apply',label:'Agent may apply a reviewed proposal',help:'Needed by proposals_apply. Writes the proposed change to the workspace and reloads the map. Every apply asks you first, in this session, showing what it would change -- but it is the irreversible half of the loop, so grant it only to an agent you would let edit the workspace.'},
 {id:'semantic:apply',label:'Agent may apply a semantic proposal',help:'The same for curated meaning: needed by semantic_proposals_apply. Asks you first, and writes when you agree.'},
 {id:'reload',label:'Agent may reload the map',help:'Needed by xyz_reload. Tells the tile service to pick up the workspace already on disk; it writes nothing and applies nothing. Useful when an apply committed but the reload was not observed. Asks you first, which is what stops repeated reloads costing the tile service anything.'},
...
export const MCP_CLIENT_PRESETS=[
 ...
 {id:'authoring',label:'Read and propose',scopes:['mcp:connect','inspect','derive','semantic:inspect','propose','semantic:propose','visual'],help:'Everything the analysis preset reads, plus the ability to add to the review queue and render evidence for it. An agent with this can draft a change, have it checked and show you what it would look like -- but not apply it. Applying is a separate scope and a separate decision.'},
 {id:'authoring-apply',label:'Author and apply',scopes:['mcp:connect','inspect','derive','semantic:inspect','propose','semantic:propose','visual','apply','semantic:apply','reload'],help:'Everything in Author, plus the ability to apply a reviewed proposal and reload the map. Every apply asks you first, in the session you are working in, and shows what it would change. Grant this to an agent you would let edit the workspace.'},
 {id:'authoring-derive',label:'Author, apply and build derived layers',scopes:['mcp:connect','inspect','derive','semantic:inspect','propose','semantic:propose','visual','apply','semantic:apply','reload','derive:manage'],help:'Everything in Author and apply, plus building and removing the derived relations layers read. The widest preset offered. Every derived-layer change asks you first, and a drop that would break a layer is refused rather than approved.'},
...
export function Security({close,hash}){
 ...
 return <div className="management-page"><section className="panel security-panel access-panel" role="region" aria-labelledby="access-title">
   ...
   <div ref={contentRef} className="security-content">
    ...
    <section id="security-cli" className="security-section" hidden={section!=='cli'}>
     ...
     <div className="token-provision">
      ...
      <p className="muted">{presetHelp}</p>
      <details><summary>Customize narrow scopes</summary><div className="token-scope-grid">{TOKEN_SCOPE_OPTIONS.map(scope=><label className="token-scope" key={scope.id}><input type="checkbox" checked={visibleScopes.includes(scope.id)} onChange={event=>toggleScope(scope.id,event.target.checked)}/><span><strong>{scope.label}</strong><small>{scope.id} · {scope.help}</small></span></label>)}</div></details>
...
export function Root(){
 const [authenticated,setAuthenticated]=useState(null),[identity,setIdentity]=useState(null),[hash,setHash]=useState(()=>window.location.hash),[area,setArea]=useState('workspace'),[visited,setVisited]=useState({}),[derived,setDerived]=useState({});
 const navigate=next=>{setArea(next);setVisited(old=>({...old,[next]:true}))};
 const requireLogin=()=>{setAuthenticated(false);setIdentity(null);setArea('workspace');setVisited({});setDerived({})};
 const check=()=>api('/api/auth/me').then(result=>{setIdentity(result);setAuthenticated(true)}).catch(()=>requireLogin());
 useEffect(()=>{window.addEventListener(AUTH_REQUIRED_EVENT,requireLogin);check();return()=>window.removeEventListener(AUTH_REQUIRED_EVENT,requireLogin)},[]);
 ...
 useEffect(()=>{if(referencedApproval(hash))navigate('security')},[hash]);
 const logout=async()=>{try{await api('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'})}finally{clearAuthentication();requireLogin()}};
 if(authenticated===null)return <p className="loading">Checking access…</p>;
 if(!authenticated)return <Login onLogin={check}/>;
 const close=()=>navigate('workspace');
 return <Dashboard activeArea={area} openWorkspace={close} onLogout={logout} openSecurity={()=>navigate('security')} openDerivedLayers={choice=>{setDerived(old=>({...choice,requestId:(old.requestId||0)+1}));navigate('derived')}} openSemantic={()=>navigate('semantic')} openFederation={()=>navigate('federation')}>
  {visited.derived&&<main className="platform-area" hidden={area!=='derived'} inert={area!=='derived'}><DerivedLayers active={area==='derived'} initialName={derived.name} initialAction={derived.action} initialRequestKey={derived.requestId} close={close}/></main>}
  {visited.semantic&&<main className="platform-area" hidden={area!=='semantic'} inert={area!=='semantic'}><SemanticCatalog api={api} identity={identity} close={close}/></main>}
  {visited.federation&&<main className="platform-area" hidden={area!=='federation'} inert={area!=='federation'}><FederatedSources api={api} close={close}/></main>}
  {visited.security&&<main className="platform-area" hidden={area!=='security'} inert={area!=='security'}><Security close={close} hash={hash}/></main>}
 </Dashboard>;
}
