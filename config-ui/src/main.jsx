...
export function Dashboard({openSecurity,openDerivedLayers,openSemantic,openFederation,onLogout=()=>{},derivedChange=null}){
 ...
 return <div className="app-shell">
  ...
  <header className="app-header">
   <button className="brand-lockup" disabled={busy} onClick={()=>{setSelected();setSidebarMode('layers')}} aria-label="Open workspace settings">
  ...
  <main className="dashboard-main" aria-busy={busy} inert={busy}>
   ...
   <aside className={`workspace-sidebar ${sidebarOpen?'open':''}`}>
    ...
    <div className="sidebar-tabs" role="group" aria-label="Workspace inventory">
     <button aria-pressed={sidebarMode==='layers'} className={sidebarMode==='layers'?'active':''} onClick={()=>setSidebarMode('layers')}><Layers3 size={16} aria-hidden="true"/>Layers</button>
     <button aria-pressed={sidebarMode==='catalog'} className={sidebarMode==='catalog'?'active':''} onClick={()=>setSidebarMode('catalog')}><Database size={16} aria-hidden="true"/>Source catalog</button>
    </div>
    {sidebarMode==='layers'?<div className="sidebar-panel" role="tabpanel">
    ...
    </div>:<div className="sidebar-panel catalog-panel" role="tabpanel">
     <div className="catalog-head"><div><span className="eyebrow">Live database</span><h2>Source catalog</h2></div><span>{catalog.length} tables</span></div>
     <SearchField label="Filter database tables" placeholder="Find a table" value={catSearch} onChange={event=>setCatSearch(event.target.value)}/>
     <div className="catalog-list">{catalog.filter(t=>`${t.dbs}.${t.schema}.${t.table}`.toLowerCase().includes(catSearch.toLowerCase())).map(t=>{const key=`${t.dbs}.${t.schema}.${t.table}`;return <button aria-pressed={selectedCatalog===key} className={`catalog-item ${selectedCatalog===key?'active':''}`} key={key} onClick={()=>setSelectedCatalog(key)}><strong>{t.schema}.{t.table}</strong><small>{t.dbs} · {t.columns.filter(c=>c.geometryType).map(c=>`${c.geometryType} · EPSG:${c.srid}`).join(', ')||'No geometry'}</small></button>})}</div>
     {selectedCatalogTable&&<div className="catalog-selection"><span><small>Selected table</small><strong>{selectedCatalogTable.schema}.{selectedCatalogTable.table}</strong></span><button disabled={namedReadOnly} title={namedReadOnly?'Switch to the default locale to add a layer.':undefined} onClick={()=>addTable(selectedCatalogTable)}>Add selected table as layer</button></div>}
    </div>}
   </aside>
