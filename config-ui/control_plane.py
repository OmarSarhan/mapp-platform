...
class ControlStore:
    ...
    def revoke_oauth_grant(
        ...
        with self._db() as connection:
            ...
            if row is not None:
                ...
                # Close legacy grant-bound windows. Client standing approvals
                # remain enabled, but only a live consent may use them.
    ...
    def disable_oauth_client(self, client_id: str, *, actor: str = "local-admin") -> bool:
        ...
        with self._db() as connection, connection.transaction():
            ...
            if row is not None:
                connection.execute(
                    "UPDATE control.approval_windows"
                    " SET revoked_at = now(), revoked_reason = 'client disabled'"
                    " WHERE client_id = %s AND revoked_at IS NULL", (client_id,),
                )
                connection.execute(
                    "UPDATE control.approvals"
                    " SET revoked_at = now(), revoked_reason = 'client disabled'"
                    " WHERE client_id = %s AND decided_by_window IS NOT NULL"
    ...
    # -- client standing approvals -------------------------------------

    WINDOW_RECENCY = dt.timedelta(minutes=15)

    def open_approval_window(
        self, *, client_id: str, instance: str, created_by: str,
        session_created_at: dt.datetime,
    ) -> dict:
        """Enable automatic decisions for this client's current permissions."""
        current = now()
        if session_created_at < current - self.WINDOW_RECENCY:
            raise ValueError("Sign in again before enabling a standing approval.")
        if not client_id or not instance:
            raise ValueError("A client and instance are required.")
        with self._db() as connection, connection.transaction():
            self._require_initialized(connection)
            client = connection.execute(
                "SELECT disabled_at, token_endpoint_auth_method"
                " FROM control.oauth_clients WHERE client_id = %s FOR UPDATE",
                (client_id,),
            ).fetchone()
            if (client is None or client["disabled_at"] is not None
                    or client["token_endpoint_auth_method"] != "none"):
                raise ValueError("Choose an enabled MCP agent client.")
            existing = connection.execute(
                "SELECT id FROM control.approval_windows"
                " WHERE client_bound AND client_id = %s AND instance = %s"
                " AND revoked_at IS NULL", (client_id, instance),
            ).fetchone()
            if existing:
            ...
            connection.execute(
                ...
                " (id, client_id, instance, client_bound, created_by,"
                " creator_auth_time, created_at) VALUES (%s,%s,%s,true,%s,%s,%s)",
                (window_id, client_id, instance, created_by,
                 session_created_at, current),
            )
        self.audit("approval.window_opened", actor=created_by, details={
            "window": window_id, "client": client_id, "instance": instance,
            "policy": "client-until-revoked",
        })
        return {"id": window_id, "enabled": True}

    def consume_approval_window(
        self, connection, *, grant_id: str, client_id: str, instance: str,
        scopes: list[str],
    ) -> dict | None:
        """Record a use under the same transaction as the resulting receipt.

        The client and live grant must agree, and both must permit the action.
        The UPDATE serializes uses with turn-off; no SKIP LOCKED means parallel
        requests do not spuriously fall back to asking for approval.
        """
        row = connection.execute(
            "UPDATE control.approval_windows w SET consumed = consumed + 1"
            " WHERE client_bound AND client_id = %s AND instance = %s"
            " AND revoked_at IS NULL AND EXISTS ("
            " SELECT 1 FROM control.oauth_grants g"
            " JOIN control.oauth_clients c ON c.client_id = g.client_id"
            " WHERE g.grant_id = %s AND g.client_id = w.client_id"
            " AND g.revoked_at IS NULL AND c.disabled_at IS NULL"
            " AND c.token_endpoint_auth_method = 'none'"
            " AND %s::text[] <@ g.scopes AND %s::text[] <@ c.scopes)"
            " RETURNING w.id, w.created_by, w.creator_auth_time, w.consumed",
            (client_id, instance, grant_id, scopes, scopes),
        ).fetchone()
        return dict(row) if row is not None else None

    def list_approval_windows(self) -> list[dict]:
        """Include every enabled client, plus recent closed approvals."""
        with self._db() as connection:
            self._require_initialized(connection)
            rows = connection.execute(
                "SELECT w.*, c.name AS client_name, c.disabled_at"
                " FROM control.approval_windows w"
                " LEFT JOIN control.oauth_clients c ON c.client_id = w.client_id"
                " WHERE w.revoked_at IS NULL OR w.revoked_at > %s"
                " ORDER BY w.created_at DESC",
                (now() - dt.timedelta(days=1),),
            ).fetchall()
        return [{**dict(row), "live": bool(
            row["client_bound"] and row["revoked_at"] is None
            and row["client_name"] is not None and row["disabled_at"] is None
        )} for row in rows]

    def revoke_approval_window(self, window_id: str, reason: str) -> bool:
        """Turn off automatic decisions and revoke their unspent receipts."""
        with self._db() as connection, connection.transaction():
            self._require_initialized(connection)
            row = connection.execute(
                "UPDATE control.approval_windows"
                " SET revoked_at = %s, revoked_reason = %s"
                " WHERE id = %s AND revoked_at IS NULL RETURNING client_id",
                (now(), reason[:200], window_id),
            ).fetchone()
            if row is None:
                return False
            connection.execute(
                "UPDATE control.approvals SET revoked_at = %s, revoked_reason = %s"
                " WHERE decided_by_window = %s AND revoked_at IS NULL"
                " AND status IN ('pending', 'approved')",
                (now(), reason[:200], window_id),
            )
        self.audit("approval.window_revoked", actor="admin", details={
            "window": window_id, "client": row["client_id"], "reason": reason[:200],
        })
        return True
    ...
    def create_approval(
        ...
        *,
        ...
        # Matching the standing approval and inserting its decision share a
        ...
        if window is not None:
            # Each decision identifies the standing approval and its running
            # use count, as well as the operator who enabled it.
            self.audit(
