...
| MCP runtime | `mapp-mcp`: the official Python SDK behind a protocol-era guard, 47 tools of which 8 write, RFC 9728 metadata, token-A authentication per call, `tools/list` filtered to what the grant can call |
...
| Operation allowlist | 49 of the platform's 69 actions |
