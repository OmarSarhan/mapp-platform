...
| MCP runtime | `mapp-mcp`: the official Python SDK behind a protocol-era guard, 52 tools of which 13 write, RFC 9728 metadata, token-A authentication per call, `tools/list` filtered to what the grant can call |
...
| Operation allowlist | 53 of the platform's 69 actions |
| Scope vocabulary | 14 accepted, derived from the allowlist plus the MCP scopes; 2 advertised in metadata |
...
Accepted scopes: `apply`, `derive`, `derive:manage`, `federation:observe`,
`federation:provision`, `inspect`, `mcp:connect`, `propose`, `reload`,
`semantic:apply`, `semantic:inspect`, `semantic:propose`, `semantic:source`,
`visual`. Derived from the operation table rather than restated, after the two
disagreed and four of the five allowlisted operations turned out to require
scopes the issuer refused to issue.

Accepted is not the same as offerable. `derive:manage`, `federation:provision`,
`federation:register`, `semantic:admin`, `semantic:generate`, `semantic:data`,
`full` and `admin` are pinned out of the dashboard, so no operator can grant
them to an agent whatever the issuer would mint.
...
| Target-client spikes (Claude, Codex, Gemini) | **3 of 3 complete** | Re-proved at 37 tools on 2026-09-18: Claude Code 2.1.276, Codex CLI 0.155.0 and Gemini CLI 0.58.0 each driven from an ephemeral container against the deployed stack, each given one task needing a four-tool chain, all three answering identically and correctly. Two decisions came out of the earlier runs and still hold: the shipped clients speak `2025-11-25` and `2025-06-18` rather than the target revision, which forced P2a, and Gemini sends no `MCP-Protocol-Version` after `initialize` -- which its own specification requires -- so the guard's header requirement was relaxed for the handshake era. The 2026-09-18 run found two defects, both fixed: a contract advertising a pagination limit the platform refuses, and `capabilities_list` refusing a tool name where an action id was wanted without naming the near match. `mcp-auth/tests/test_registered_client.py` still drives the authorization column against the real component |
